...
@app.get("/")
async def root():
    return {"status": "online", "message": "RotaAI is running on Render!", "version": "4.1.0"}

# 2. HEALTH CHECK
@app.get("/api/health")
async def health():
    info = {"status": "online", "version": "4.1.0", "platform": "Render"}
    try:
        ...
        if db:
            ...
            info["db_error"] = os.environ.get("FIREBASE_INIT_ERROR", "Initialization failed")
